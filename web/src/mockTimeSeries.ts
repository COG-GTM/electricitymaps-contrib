...
/**
 * Build a 48-hour synthetic hourly series for a zone, anchored to its current
 * mock production. Solar follows a daylight curve, wind uses a sinusoid, and
 * fossil fuels follow demand. Carbon intensity is recomputed from the resulting mix.
 */
export function generateTimeSeries(
  ...
  for (let i = hours; i >= 0; i--) {
    ...
    const windFactor =
      0.5 + 0.5 * Math.sin((hour / 24) * Math.PI * 2 + pseudoRandom(seed + i) * 2);
    ...
    for (const [mode, base] of Object.entries(baseData.production)) {
      ...
      if (SOLAR_MODES.has(mode)) {
        ...
        val =
          base *
          windFactor *
          (0.5 + pseudoRandom(seed + i * 3 + modeIdx) * 1);
      } else if (DEMAND_MODES.has(mode)) {
        val =
          base *
          demandFactor *
          (0.75 + pseudoRandom(seed + i * 7 + modeIdx) * 0.5);
      } else if (STABLE_MODES.has(mode)) {
        val = base * (0.92 + pseudoRandom(seed + i * 11 + modeIdx) * 0.16);
      } else {
        val = base * (0.7 + pseudoRandom(seed + i * 13 + modeIdx) * 0.6);
      }
